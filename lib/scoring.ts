import { FactorKey, FactorScores, ReadinessCategory } from '@/types';

/**
 * Weights for each dimension (percentages that sum to 100)
 */
export const DIMENSION_WEIGHTS: Record<FactorKey, number> = {
  motivation: 15,
  career: 15,
  search: 15,
  employability: 15,
 ...
 */
export function readinessIndex(scores: FactorScores): number {
  let total = 0;
  
  for (const key of Object.keys(scores) as FactorKey[]) {
 ...
 * B = everything else
 */
export function readinessCategory(scores: FactorScores): ReadinessCategory {
